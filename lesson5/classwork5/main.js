...
const numberMin = (num1, num2, num3) => {
...
const numberMax = (number1, number2, number3) =>
{
...
const numRandom = (lenght) => {
    let arrayVacant = [];
    for (let i = 0; i < lenght; i++) {
        arrayVacant.push(Math.floor(Math.random()*100));
    } return arrayVacant;
}
document.write(numRandom (6));

const numberRandom = (lenght, limit) => {
    let arrayEmpty = [];
    for (let i = 0; i < lenght; i++) {
        arrayEmpty.push(Math.floor(Math.random()* limit));
    } return arrayEmpty;
}
document.write( numberRandom (4,50));
